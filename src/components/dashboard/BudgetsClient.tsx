...
export function BudgetsClient({ initialBudgets, initialCachedPlan, month, year }: BudgetsClientProps) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 pb-28 md:pb-8">
      ...
      {/* Stats Summary Card */}
      {totalBudgeted > 0 && (
        <div className="bg-white text-black border-2 border-black dark:border-white rounded-2xl p-5 shadow-[4px_4px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_rgba(255,255,255,0.15)] transition-all mb-2">
          <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-5">
            <div className="space-y-1">
          ...
          <div className="h-4 sm:h-5 w-full flex rounded-full overflow-hidden border-2 border-black dark:border-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] bg-slate-100 dark:bg-zinc-800">
            {displayBudgets.filter(b => b.budget_limit > 0).map((b) => {
              const width = (b.budget_limit / totalBudgeted) * 100
              return (
                <div 
                  key={b.category_id}
                  className="h-full border-r-2 border-black dark:border-white last:border-r-0 transition-all hover:brightness-110"
          ...
          <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2">
            {displayBudgets.filter(b => b.budget_limit > 0).map((b) => {
              const percentage = Math.round((b.budget_limit / totalBudgeted) * 100)
              return (
                <div key={`legend-${b.category_id}`} className="flex items-center gap-1.5 text-[10px] font-bold">
                  <div className="w-2.5 h-2.5 rounded-full border border-black dark:border-white" style={{ backgroundColor: b.category_color }} />
      ...
      {displayBudgets.length === 0 ? (
        ...
        <div className="bg-white dark:bg-zinc-950 border-2 border-black dark:border-white rounded-2xl shadow-[4px_4px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_rgba(255,255,255,0.15)] overflow-hidden divide-y-2 divide-black/10 dark:divide-white/10 mt-2">
          {displayBudgets.map((b) => {
            ...
            let badgeColorClass = 'bg-[#c8e6cd] text-black'
            
            if (percentage >= 100) {
              badgeColorClass = 'bg-[#f4cfd4] text-black'
            } else if (percentage >= 80) {
              badgeColorClass = 'bg-[#ffe8b2] text-black'
            }
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  reviewMode ? "bg-slate-50/50 dark:bg-zinc-900/50 hover:bg-slate-50 dark:hover:bg-zinc-900" : "hover:bg-slate-50 dark:hover:bg-zinc-900",
              ...
              >
                ...
                <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
                  <div 
                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border-2 border-black dark:border-white shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_rgba(255,255,255,0.15)] shrink-0"
                    style={{ backgroundColor: b.category_color }}
                  >
                    <Icon className="h-4 w-4 sm:h-5 sm:w-5 text-black" />
                  </div>
                  <div className="min-w-0">
                    <h4 className="font-extrabold text-sm sm:text-base text-black dark:text-white truncate flex items-center gap-2">
                      {b.category_name}
                      {b.category_id.startsWith('new-') && (
                        <span className="bg-[#c5b0f4] text-black text-[8px] px-2 py-0.5 rounded-full uppercase tracking-wider border border-black">Baru</span>
                      )}
                    </h4>
                    {b.budget_notes && !reviewMode && (
                      <p className="text-[10px] text-slate-500 italic truncate max-w-[200px] mt-0.5">
                        &quot;{b.budget_notes}&quot;
                      </p>
                    )}
                    {reviewMode && plan?.budgets?.find(ab => ab.category_id === b.category_id) && (
                      <p className="text-[10px] font-bold text-[#8c52ff] mt-0.5 flex items-center gap-1">
                        <LucideIcons.Sparkles className="h-3 w-3" />
                        AI Saran: {formatRupiah(plan.budgets.find(ab => ab.category_id === b.category_id)!.recommended_limit)}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  {reviewMode ? (
                    <div className="relative w-full sm:w-[180px]">
                      ...
                      <Input
                        ...
                        className="pl-8 py-2 h-10 text-sm font-black bg-white dark:bg-zinc-950 border-2 border-black dark:border-white rounded-xl shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_rgba(255,255,255,0.15)] focus-visible:ring-0 focus-visible:shadow-[1px_1px_0px_rgba(0,0,0,1)] focus-visible:translate-x-[1px] focus-visible:translate-y-[1px] transition-all"
                      />
                    </div>
                  ) : (
                      ...
                      {hasBudget ? (
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <div className="font-extrabold text-xs sm:text-sm text-black dark:text-white">
                          ...
                          <span className={cn(
                            "px-2 py-1 rounded-lg text-[10px] font-extrabold border-2 border-black dark:border-white shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_rgba(255,255,255,0.15)]",
                            badgeColorClass
                          )}>
                            {percentage}%
                          </span>
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-2 border-dashed border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-lg">
                          Belum diatur
                        </span>
                      )}
                      ...
                      <BudgetForm
                        ...
                        trigger={
                          <Button 
                            variant="outline" 
                            size="icon" 
                            className="h-9 w-9 rounded-xl border-2 border-black dark:border-white shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_rgba(255,255,255,0.15)] hover:bg-slate-100 dark:hover:bg-zinc-800 shrink-0"
                          >
                            {hasBudget ? (
                              <LucideIcons.Edit2 className="h-4 w-4 text-black dark:text-white" />
                            ) : (
                              <LucideIcons.Plus className="h-4 w-4 text-black dark:text-white" />
                            )}
                          </Button>
                        }
                      />
